...
```bash
# 单节点 (32K 上下文, TP=8)
bash examples/glm-5_w4a8/vllm/run_vllm.sh

# 2 节点大 TP (202K 上下文)
TP=16 MAX_MODEL_LEN=202752 bash examples/glm-5_w4a8/vllm/run_vllm.sh

# 后台运行
nohup bash examples/glm-5_w4a8/vllm/run_vllm.sh > glm5_vllm.log 2>&1 &

# 使用传统包装器部署
bash examples/glm-5_w4a8/vllm/vllm_server.sh

# 无 MTP 版本 (节省内存)
bash examples/glm-5_w4a8/vllm/run_vllm_nomtp.sh
```
...
```bash
# 运行测试脚本
bash examples/glm-5_w4a8/vllm/curl_test.sh
