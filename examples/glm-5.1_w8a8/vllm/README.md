...
bash examples/glm-5.1_w8a8/vllm/run_vllm.sh

# 大上下文
TP=8 PP=2 MAX_MODEL_LEN=131072 bash examples/glm-5.1_w8a8/vllm/run_vllm.sh

# 打开 MTP 投机解码（仅 PP=1 时可用）
ENABLE_MTP=1 bash examples/glm-5.1_w8a8/vllm/run_vllm.sh

# 后台运行
nohup bash examples/glm-5.1_w8a8/vllm/run_vllm.sh > glm5_1_w8a8_vllm.log 2>&1 &
```

### 验证

```bash
bash examples/glm-5.1_w8a8/vllm/curl_test.sh
