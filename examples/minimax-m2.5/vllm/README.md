...
```bash
# 单节点 A2 (32K 上下文, TP=8)
bash examples/minimax-m2.5/vllm/run_vllm.sh

# A3 16 卡 (更大上下文)
TP=8 MAX_MODEL_LEN=65536 bash examples/minimax-m2.5/vllm/run_vllm.sh

# 多节点
TP=8 PP=2 bash examples/minimax-m2.5/vllm/run_vllm.sh

# 后台运行
nohup bash examples/minimax-m2.5/vllm/run_vllm.sh > minimax_m25_vllm.log 2>&1 &
```
...
```bash
# 运行测试脚本
bash examples/minimax-m2.5/vllm/curl_test.sh
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MODEL_PATH` | `/home/jianzhnie/llmtuner/hfhub/models/MiniMax/MiniMax-M2.5` | 模型权重路径 |
| `SERVED_MODEL_NAME` | `minimax-m2.5` | API 中的模型名称 |
| `HOST` | `0.0.0.0` | 监听地址 |
| `PORT` | `8006` | 监听端口 |
