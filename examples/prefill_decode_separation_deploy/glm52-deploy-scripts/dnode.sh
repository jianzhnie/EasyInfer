#!/bin/bash
# ==============================================================================
# dnode.sh — Decode 节点统一启动模板 (aligned with official GLM-5.2 PD docs)
# ==============================================================================
...
# ---- 环境变量 (official GLM-5.2 PD docs, decode node) -----------------------
...
# ---- 确保日志目录存在 --------------------------------------------------------
mkdir -p "$log_dir"
LOG_FILE="${log_dir}/glm5_decode_$(date +%Y%m%d_%H%M%S)_rank${dp_rank}.log"

# ---- vLLM serve (Decode / KV Consumer, official config) ---------------------
nohup vllm serve "$model_path" \
    ...
    --trust-remote-code \
    --max-num-seqs "${D_MAX_NUM_SEQS:-32}" \
