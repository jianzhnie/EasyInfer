...
| 文件 | 用途 |
|------|------|
| `run_sglang.sh` | 部署/停止/查看状态/查看日志 (在管理节点上执行) |
| `test_sglang.sh` | API 功能验证 |
| `lm_eval.sh` | 模型评测 (lm-evaluation-harness) |
...
```bash
bash run_sglang.sh --status
```

输出示例：
```
  10.42.11.130: 运行中 (9 个进程)
  10.42.11.131: 运行中 (9 个进程)
  ...
```
...
```bash
# 查看主节点最后 50 行日志
bash run_sglang.sh --logs

# 查看主节点最后 200 行日志
bash run_sglang.sh --logs 200
```

### 重启服务

```bash
bash run_sglang.sh --restart
```
...
# 指定评测任务
TASKS=gsm8k,ceval-valid bash examples/longcat/lm_eval.sh

# 指定其他参数
API_HOST=10.42.11.130 PORT=6677 TASKS=mmlu FEWSHOT=5 bash examples/longcat/lm_eval.sh
```
...
| `SGLANG_LOG_DIR` | `/tmp` | 日志目录 (容器内与 host 一致) |
...
- **日志统一存放在 `/tmp/sglang_${MODEL_NAME}.log`**，容器内与 host 路径一致
...
A: 模型加载通常需要 10-20 分钟（148 个 safetensors 分片 + 64 卡 HCCL 初始化）。查看日志确认进度：
```bash
bash run_sglang.sh --logs
```
...
### Q: 评测时 `lm-eval` 命令未找到
A: 在管理节点上安装 lm-evaluation-harness：
```bash
pip3 install lm-eval[api]
```

### Q: 评测时连接服务超时
A: 如果管理节点配置了 HTTP 代理，脚本会自动将 `API_HOST` 加入 `no_proxy`。如仍有问题，手动设置：
```bash
export no_proxy="${no_proxy},10.42.11.130"
bash examples/longcat/lm_eval.sh
```
