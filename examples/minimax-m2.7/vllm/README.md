...
bash examples/minimax-m2.7/vllm/run_vllm.sh

# A3 16 卡 (更大上下文)
TP=8 MAX_MODEL_LEN=65536 bash examples/minimax-m2.7/vllm/run_vllm.sh

# 多节点
TP=8 PP=2 bash examples/minimax-m2.7/vllm/run_vllm.sh

# 后台运行
nohup bash examples/minimax-m2.7/vllm/run_vllm.sh > minimax_m27_vllm.log 2>&1 &

# 使用传统包装器部署
bash examples/minimax-m2.7/vllm/vllm_server.sh
```
...
```bash
# 运行测试脚本
bash examples/minimax-m2.7/vllm/curl_test.sh
