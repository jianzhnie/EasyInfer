...
| B（保守，对齐官方） | TP=8 PP=2（2 节点） | DP=8, PCP=1 DCP=8 | ❌（PP>1 互斥） | 权重 ~30G/卡，KV 余量小，无 MTP |
...
- **TP=8 是唯一选择**：TP=16 因 MLA 维度（576）不可整除不可用；TP 保持在节点内（8 卡 NVLink 级互联）。
...
| Claude Code / Agent 接入 | ✅ | `agent_api.sh` |
...
- **精度评估**：使用 AISBench，详见官方文档「使用 AISBench」章节（lm_eval 方式官方尚未验证）
