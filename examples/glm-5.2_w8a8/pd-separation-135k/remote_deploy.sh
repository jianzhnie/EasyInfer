...
#   P: 10.42.11.202-205 :9081  TP=8 DP=4 (每节点 1 rank, enforce-eager)
#   D: 10.42.11.206-209 :9900  TP=4 DP=8 (每节点 2 rank, FULL_DECODE_ONLY)
#   KV: MooncakeConnector (kv_p2p, use_ascend_direct)  代理: 202:8123
#
# Usage:
...
wait_ready() {  # $1=ip $2=port $3=label
    ...
    while true; do
        local code
        code=$(ssh_run "$ip" "docker exec $CONTAINER_NAME bash -c 'curl -s -o /dev/null -w %{http_code} --max-time 5 http://localhost:$port/v1/models'" 2>/dev/null | tail -1)
...
cmd_deploy() {
    ...
    for i in 0 1 2 3; do wait_ready "${DNODE_IPS[$i]}" "$D_VLLM_START_PORT" "DNode$i" & done
    wait
...
cmd_status() {
    for i in 0 1 2 3; do
        ...
        ssh_run "${PNODE_IPS[$i]}" "docker exec $CONTAINER_NAME bash -c 'curl -s -o /dev/null -w %{http_code} --max-time 5 http://localhost:$P_VLLM_START_PORT/v1/models'" 2>/dev/null | tail -1; echo
    done
    for i in 0 1 2 3; do
        printf "  DNode%s %-15s :%s " "$i" "${DNODE_IPS[$i]}" "$D_VLLM_START_PORT"
        ssh_run "${DNODE_IPS[$i]}" "docker exec $CONTAINER_NAME bash -c 'curl -s -o /dev/null -w %{http_code} --max-time 5 http://localhost:$D_VLLM_START_PORT/v1/models'" 2>/dev/null | tail -1; echo
    done
}
...
cmd_stop() {
    ...
    for ip in "${PNODE_IPS[@]}" "${DNODE_IPS[@]}"; do
        printf "  %s ... " "$ip"
        ssh_run "$ip" "docker exec $CONTAINER_NAME bash -c 'pkill -f \"vllm serve\" || true'" && echo -e "${GREEN}stopped${NC}"
    done
    echo "完成"
}
...
cmd_proxy() {
    if ssh_run "${PNODE_IPS[0]}" "docker exec $CONTAINER_NAME bash -c 'pgrep -f load_balance_proxy_server_example >/dev/null'" 2>/dev/null; then
    ...
    local d_ports=("$D_VLLM_START_PORT" "$D_VLLM_START_PORT" "$D_VLLM_START_PORT" "$D_VLLM_START_PORT")
    ssh_run "${PNODE_IPS[0]}" "docker exec -d $CONTAINER_NAME bash -c '
        ...
        nohup python3 $PROXY_SCRIPT \
            ...
            --decoder-hosts ${DNODE_IPS[*]} --decoder-ports ${d_ports[*]} \
            > $LOG_DIR/pd135k_proxy.log 2>&1 &'"
    echo -e "${GREEN}代理已启动: ${PNODE_IPS[0]}:${PROXY_PORT}${NC}  日志: $LOG_DIR/pd135k_proxy.log"
}

cmd_proxy-stop() {
    ssh_run "${PNODE_IPS[0]}" "docker exec $CONTAINER_NAME bash -c 'pkill -f load_balance_proxy_server_example'" \
